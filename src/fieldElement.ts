...
export class FieldElement {
	...
	static FromBytes(bytes: Uint8Array): FieldElement {
		...
		res.data[1] =
			(uint8ArrayToBigIntLE(bytes.slice(6, 6 + 8)) >> 3n) & LOW_51_BIT_MASK;
		// // load bits [ 96,160), shift to [102,160)
		res.data[2] =
			(uint8ArrayToBigIntLE(bytes.slice(12, 12 + 8)) >> 6n) & LOW_51_BIT_MASK;
		// // load bits [152,216), shift to [153,216)
		res.data[3] =
			(uint8ArrayToBigIntLE(bytes.slice(19, 19 + 8)) >> 1n) & LOW_51_BIT_MASK;
		// // load bits [192,256), shift to [204,112)
		res.data[4] =
			(uint8ArrayToBigIntLE(bytes.slice(24, 24 + 8)) >> 12n) & LOW_51_BIT_MASK;
		return res;
	}
