...
export class SecretKey {
	...
	static FromBytes(bytes: Uint8Array): SecretKey {
		if (bytes.length !== 64) {
			throw new Error(
				`Invalid secret key length: ${bytes.length} (expected 64)`,
			);
		}
	...
	static FromMiniSecret(miniSecret: Uint8Array): SecretKey {
		if (miniSecret.length !== 32) {
			throw new Error(
				`Invalid mini secret length: ${miniSecret.length} (expected 32)`,
			);
		}
	...
	sign(
		...
		const signingContext = new SigningContext085(
			textEncoder.encode("substrate"),
		);
...
export class PublicKey {
	...
	verify(message: Uint8Array, signatureBytes: Uint8Array): boolean {
		const signingTranscript = new SigningContext085(
			textEncoder.encode("substrate"),
		).BytesClone(message);
		...
		signingTranscript.AppendMessage(
			textEncoder.encode("proto-name"),
			textEncoder.encode("Schnorr-sig"),
		);
		signingTranscript.AppendMessage(textEncoder.encode("sign:pk"), this.key);
		signingTranscript.AppendMessage(
			textEncoder.encode("sign:R"),
			signature.R.ToBytes(),
		);

		const k = Scalar.FromBytes(
			Scalar.FromBytesModOrderWide(
...
class Signature {
	...
	static FromCompressedRistrettoAndScalar(
	...
	ToBytes() {
		...
		const mergedArray = new Uint8Array(
			compressedRistrettoBytes.length + scalarBytes.length,
		);
