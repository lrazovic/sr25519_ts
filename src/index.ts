...
} from "../translated/mnemonic/mnemonic";
import { PublicKey } from "./signingContext";
import type {
	IUniqueSdkSigner,
	UNIQUE_SDK_SignTxResultResponse,
	UNIQUE_SDK_UnsignedTxPayloadBody,
} from "./types";
	...
	return {
		...
		sign(message: Uint8Array | string): Uint8Array {
			return keypair.secretKey
				.sign(anyToU8a(message), keypair.publicKey)
				.ToBytes();
		},
...
export const verifySignature = (
	message: Uint8Array | string,
	signature: Uint8Array | string,
	signerAddressOrPublicKey: Uint8Array | string,
) => {
...
export const Sr25519Account = {
	...
	other: {
		...
		fromSecretKeyBytes: (secretKeyBytes: Uint8Array | string) => {
			const keypair = SecretKeypair.FromSecretKeyBytes(
				u8aOrHexToU8a(secretKeyBytes),
			);
			return getAccountFromKeypair(keypair);
		},
