...
describe("main test", async () => {
	...
	test("secretKey as in polkadot lib", () => {
		const miniSecretFromPolkadot =
			utilCrypto.mnemonicToMiniSecret(DEFAULT_MNEMONIC);
		...
		const keypairFromPolkadot = utilCrypto.sr25519PairFromSeed(
			miniSecretFromPolkadot,
		);
		const keypairFromOurLib =
			Account.other.dangerouslyParseUriAndGetFullKeypair(DEFAULT_MNEMONIC);

		expect(keypairFromPolkadot.publicKey).toEqual(
			keypairFromOurLib.publicKey.key,
		);
		expect(keypairFromPolkadot.secretKey).toEqual(
			keypairFromOurLib.secretKey.ToBytes(),
		);
	});
	...
	test("sign with a signer for Unique SDK", async () => {
		const account = Account.fromUri(Alice.uri);
		const message = b`hello world`;
		const signResult = await account.signer.sign({
			signerPayloadHex: uInt8ArrayToHex(message),
		});
	...
	test("verify", () => {
		const keypair = Account.other.dangerouslyParseUriAndGetFullKeypair(
			Alice.uri,
		);
		const account = Account.fromUri(Alice.uri);
		...
		const message = b`hello world`;
		const signature = utilCrypto.sr25519Sign(message, {
			publicKey,
			secretKey: keypair.secretKey.ToBytes(),
		});
		const isValidWithPolkadotVerifier = utilCrypto.sr25519Verify(
			message,
			signature,
			publicKey,
		);
	...
	test("verify quick test", () => {
		...
		expect(
			utilCrypto.sr25519Verify(message, signature, account.publicKey),
		).toBe(true);
		expect(account.verify(message, signature)).toBe(true);
	});
...
describe("extra tests", async () => {
	test("sign with random keys", async () => {
		...
		const signature = keypair.secretKey
			.sign(message, keypair.publicKey)
			.ToBytes();
