...
import {
	...
	isHex,
	u8aConcatStrict,
} from "../translated/mnemonic/bytes";
...
export const encodeSubstrateAddress = (
	key: Uint8Array,
	ss58Format = 42,
): string => {
	if (ss58Format < 0 || ss58Format > 16383 || [46, 47].includes(ss58Format)) {
		throw new Error(
			`ss58Format is not valid, received ${typeof ss58Format} "${ss58Format}"`,
		);
	}
	...
	if (!allowedDecodedLengths.includes(key.length)) {
		throw new Error(
			`key length is not valid, received ${key.length}, valid values are ${allowedDecodedLengths.join(", ")}`,
		);
	}
...
export function decodeSubstrateAddress(
	address: string,
	...
	try {
		...
		if (!allowedEncodedLengths.includes(decoded.length)) {
			realError = new Error(
				`key length is not valid, decoded key length is ${decoded.length}, valid values are ${allowedEncodedLengths.join(", ")}`,
			);
			throw realError;
		}

		const [isValid, endPos, ss58Length, ss58Decoded] = checkAddressChecksum(
			decoded,
			ignoreChecksum,
		);
